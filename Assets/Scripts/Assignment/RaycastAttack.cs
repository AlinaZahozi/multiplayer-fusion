...
public class RaycastAttack : NetworkBehaviour
{
    ...
    private Score playerScore; // Reference to the player's Score component

    private void Awake()
    {
        playerScore = GetComponent<Score>(); // Initialize the playerScore reference
    }
        ...
        if (attack.WasPerformedThisFrame())
        {
            ...
            if (Runner.GetPhysicsScene().Raycast(ray.origin, ray.direction * shootDistance, out var hit))
            {
                ...
                if (hitObject.TryGetComponent<Health>(out var health))
                {
                    ...
                    // Check if the playerScore component is not null
                    if (playerScore != null)
                    {
                        // Update the player's score for hitting an opponent
